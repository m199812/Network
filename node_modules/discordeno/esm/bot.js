...
import { baseEndpoints, CHANNEL_MENTION_REGEX, CONTEXT_MENU_COMMANDS_NAME_REGEX, DISCORD_SNOWFLAKE_REGEX, DISCORDENO_VERSION, endpoints, SLASH_COMMANDS_NAME_REGEX, USER_AGENT, } from "./util/constants.js";
import { createGatewayManager } from "./gateway/mod.js";
...
import { Errors, GatewayIntents } from "./types/shared.js";
...
export function createBot(options) {
    ...
    bot.gateway = createGatewayManager({
        ...
        handleDiscordPayload: bot.handleDiscordPayload ??
            async function (_, data, shardId) {
                // TRIGGER RAW EVENT
                bot.events.raw(bot, data, shardId);
                if (!data.t)
                    return;
                // RUN DISPATCH CHECK
                await bot.events.dispatchRequirements(bot, data, shardId);
                bot.handlers[data.t]?.(bot, data, shardId);
            },
    });
    return bot;
}
