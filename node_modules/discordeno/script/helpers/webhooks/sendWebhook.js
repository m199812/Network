...
async function sendWebhook(bot, webhookId, webhookToken, options) {
    const allowedMentions = options.allowedMentions
        ...
        : { parse: [] };
    const result = await bot.rest.runMethod(bot.rest, "post", `${bot.constants.endpoints.WEBHOOK(webhookId, webhookToken)}?wait=${options.wait ?? false}${options.threadId ? `&thread_id=${options.threadId}` : ""}`, {
        wait: options.wait,
