...
exports.spawnShards = exports.prepareBuckets = void 0;
function prepareBuckets(gateway, firstShardId, lastShardId) {
    ...
    for (let i = firstShardId; i < lastShardId; i++) {
        ...
        if (!bucket)
            throw new Error("Bucket not found when spawning shards.");
        ...
        if (queue) {
            ...
            if (worker + 1 <= gateway.maxWorkers)
                worker++;
...
function spawnShards(gateway, firstShardId = 0) {
